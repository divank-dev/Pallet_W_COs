...
CREATE TYPE production_method AS ENUM ('ScreenPrint', 'Embroidery', 'DTF', 'Other');
CREATE TYPE art_status AS ENUM ('Not Started', 'In Progress', 'Sent to Customer', 'Revision Requested', 'Approved');
CREATE TYPE lead_source AS ENUM ('Website', 'Referral', 'Social Media', 'Cold Call', 'Trade Show', 'Email Campaign', 'Other');
CREATE TYPE lead_temperature AS ENUM ('Hot', 'Warm', 'Cold');
CREATE TYPE fulfillment_method AS ENUM ('Shipped', 'PickedUp');
CREATE TYPE user_role AS ENUM ('Admin', 'Manager', 'Sales', 'Production', 'Fulfillment', 'ReadOnly');
CREATE TYPE payment_method AS ENUM ('Cash', 'Check', 'Credit Card', 'ACH', 'Other');
CREATE TYPE art_file_type AS ENUM ('original', 'proof', 'markup', 'reference', 'final');
CREATE TYPE art_file_source AS ENUM ('client', 'designer', 'system');
CREATE TYPE proof_status AS ENUM ('Draft', 'Sent', 'Approved', 'Revision Needed');
...
CREATE TABLE orders (
  ...
  search_vector TSVECTOR GENERATED ALWAYS AS (
    to_tsvector('english', COALESCE(order_number, '') || ' ' || COALESCE(customer_name, '') || ' ' || COALESCE(project_name, '') || ' ' || COALESCE(customer_email, ''))
  ) STORED
);
...
CREATE POLICY "Users: Admins full access" ON users FOR ALL TO authenticated USING (get_user_role() = 'Admin');
CREATE POLICY "Users: Read all" ON users FOR SELECT TO authenticated USING (true);
CREATE POLICY "Users: Update self" ON users FOR UPDATE TO authenticated USING (auth_user_id = auth.uid());
CREATE POLICY "Customers: Read all" ON customers FOR SELECT TO authenticated USING (true);
CREATE POLICY "Customers: Sales/Admin modify" ON customers FOR ALL TO authenticated USING (get_user_role() IN ('Admin', 'Manager', 'Sales'));
CREATE POLICY "Products: Read all" ON products FOR SELECT TO authenticated USING (true);
CREATE POLICY "Products: Admin/Manager modify" ON products FOR ALL TO authenticated USING (get_user_role() IN ('Admin', 'Manager'));
CREATE POLICY "Orders: Read all" ON orders FOR SELECT TO authenticated USING (true);
CREATE POLICY "Orders: Non-ReadOnly modify" ON orders FOR ALL TO authenticated USING (get_user_role() != 'ReadOnly');
CREATE POLICY "Line Items: Read all" ON line_items FOR SELECT TO authenticated USING (true);
CREATE POLICY "Line Items: Non-ReadOnly modify" ON line_items FOR ALL TO authenticated USING (get_user_role() != 'ReadOnly');
CREATE POLICY "Logs: Read all" ON status_change_logs FOR SELECT TO authenticated USING (true);
CREATE POLICY "Logs: Insert all" ON status_change_logs FOR INSERT TO authenticated WITH CHECK (true);
CREATE POLICY "Productivity: Read all" ON productivity_entries FOR SELECT TO authenticated USING (true);
CREATE POLICY "Productivity: Production/Admin modify" ON productivity_entries FOR ALL TO authenticated USING (get_user_role() IN ('Admin', 'Manager', 'Production'));
CREATE POLICY "Art Files: Read all" ON art_files FOR SELECT TO authenticated USING (true);
...
SELECT o.id, o.order_number, o.customer_name, o.project_name, o.status, o.art_status, o.due_date, o.rush_order, o.created_at, o.updated_at,
  COUNT(DISTINCT li.id) as line_item_count, SUM(li.qty) as total_quantity, SUM(li.price * li.qty) as total_revenue,
  SUM(li.cost * li.qty) as total_cost, SUM((li.price - li.cost) * li.qty) as total_profit
FROM orders o LEFT JOIN line_items li ON li.order_id = o.id WHERE NOT o.is_archived GROUP BY o.id;

CREATE VIEW daily_productivity_summary AS
SELECT date, SUM(items_decorated) as total_decorated, SUM(items_packed) as total_packed,
  COUNT(DISTINCT operator_name) as operator_count, COUNT(*) as entry_count
FROM productivity_entries GROUP BY date ORDER BY date DESC;
